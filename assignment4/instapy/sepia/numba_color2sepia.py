...
def numba_color2sepia(image_array):
    ...
    return _conversion(image_array, sepia_array).astype("uint8")

@jit
def _conversion(image_array, sepia_array):
    # Converts the values in the images' channels to sepia values
    for row_index, row in enumerate(image_array):
        for column_index, column in enumerate(row):
            ...
            for channel in range(3):
                ...
                sepia_array[row_index, column_index,
                            channel] = weighted_sum if weighted_sum < 255 else 255
    return sepia_array
